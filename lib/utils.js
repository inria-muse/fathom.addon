...
exports.nsprFile = (function() { 
    ...
    console.log("utils xullAppInfo="+xulAppInfo.version+" nsprFile=" + libname);
    ...
    console.log("utils check " + libfile.path);
    if (libfile.exists()) {
        console.log("utils found!");    
        return libfile;    
    }    

    // FIXME: the old way -- remove if above works for all ?
    if (!isAndroid()) {
		var libd = "LibD";
		if (isDarwin())
		    libd = "ULibDir";
		...
		for (var i in dirs) {
            ...
            console.log("utils check " + dirs[i].path);
		    if (!dirs[i].exists())
				continue;
        ...
        _.each(['fennec-xx.apk', 'fennec-xx/base.apk', 'firefox-xx.apk', 'firefox-xx/base.apk'], function(base) {
            ...
            for (var i = 1; i < 4; i++) {
    		    try {
                    ...
                    console.log("utils check " + basepath);

    				var f = FileUtils.File(basepath);
	       			if (!f.exists()) {
                        continue;
                    }
                    console.log("utils basepath found!");                
...
/** Resolve basic IP info using RIPE stats API. If no IP given, will first try to lookup current public IP. */
