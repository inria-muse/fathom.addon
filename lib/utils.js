...
var isExecFile = exports.isExecFile = function(str) {
  try {
    var file = Cc['@mozilla.org/file/local;1'].createInstance(getLocalFileApi());
...
exports.nsprFile = (function() {
  ...
  var psm = Cc["@mozilla.org/psm;1"].getService(Ci.nsISupports);
  ...
  if (!libfile.exists()) {
    ...
    if (!isAndroid()) {
      ...
      if (!libfile.exists()) {
        ...
        _.each(['fennec-xx.apk', 'fennec-xx/base.apk', 'firefox-xx', 'firefox-xx/base.apk'], function(base) {
          ...
          for (var i = 1; i < 4; i++) {
            ...
            try {
              // Try to open the lib. The nsIFile.exists() method does not work in this case because the lib is inside the APK.
              if (ctypes.open(libfile.path)) {
                break;
              }
            } catch (e) {
              // Nothing: the lib cannot be openned
            }
...
var lookupMAC = exports.lookupMAC = function(callback, mac) {
  ...
  getJSON(function(json) {
    ...
    if (_.size(maccache) > MAX_CACHE) {
      ...
      _.each(_.sortBy(_.keys(maccache), function(k) {
        return maccache[k].ts; // oldest first
      }).slice(0, int(MAX_CACHE*0.5)), function(k) {
        delete maccache[k];
      }); // _.each
    }
