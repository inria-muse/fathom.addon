...
const GW_SERVICE_MATCH = ['openwrt'];
...
var MDNSResponse = exports.MDNSResponse = function(address) {
    ...
    // some flags we can infer from mdns
    this.isgw = false;         // is a gateway device ?
    this.isfathom = false;     // has fathom APIs ?
    this.iswin = false;        // running windows ?
    this.islinux = false;      // running linux ?
    this.isdarwin = false;      // running darwin / os x  ?
};
...
MDNSResponse.prototype.update = function(a) {
	...
	if (!this.isfathom)
	    this.isfathom = (name.toLowerCase().indexOf('fathom')>=0);

	if (!this.islinux)
	    this.islinux = (name.toLowerCase().indexOf('linux')>=0);
