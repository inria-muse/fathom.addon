...
/** Store a page baseline measurement */
DB.prototype.savePageload = function(p, cb) {
    if (!this.isConnected()) {
	if (cb) cb(error("internal","db not available"));
	return;
    }

    console.debug(p);
    
    // TODO: store stats about pageload to the baseline tables
    // - number of pageloads, reloads and hist navigation
    // - last 2min mean DNS resolution time
    // - last 2min mean TCP conn time
    // - last 2min mean TCP SSL conn time
    // - last 2min mean download time
    // - last 2min mean network time
    // - last 2min mean DOM load time
    // - last 2min mean total pageload time
    
    // TODO: store stats for top-k alexa / user pages ?
...
DB.prototype.updateEnvUserLabel = function(envid, label, cb) {
    if (!this.isConnected()) return cb(error("internal","db not available"));
...
/** Get environment timeseries with full env info. */
DB.prototype.getEnvRange = function(range, cb) {
    ...
    this.conn.execute(stmt, null, function(row) {
	let o = {};
...
DB.prototype.getBaselineRange = function(metrics, range, cb) {
    if (!this.isConnected()) return cb(error("internal","db not available"));
