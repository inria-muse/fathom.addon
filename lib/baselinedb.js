...
DB.prototype.connect = function(cb) {
    ...
    Task.spawn(function() {
        ...
        if (!initdone) {
            ...
            // the DB version
            console.log('check for DB updates');

            // updates 5&6&7 require resetting the database (bad values)
            if (that.version < 7) {
                ...
                ss.storage['baseline_dbschema'] = 7;
                that.version = 7;
...
DB.prototype.saveBaseline = function(bs) {
    ...
    if (!bs.mem.error && o.mem_total == null) {
        o.mem_total = bs.load.result.memtotal; 
        o.mem_used = bs.load.result.memused;
        o.mem_free = bs.load.result.memfree;        
    }
