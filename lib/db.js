...
DB.prototype.connect = function(cb) {
    if (this.conn) {
        console.warn('baselinedb already connected!');
        if (cb) cb();
        return;
    }
    ...
    Task.spawn(function() {
        ...
        if (!initdone) {
            ...
            if (that.version < 7 || (that.version < 8 && utils.isWin())) {
                ...
                yield that.conn.execute("ALTER TABLE env ADD COL mserver_ip text;")
                yield that.conn.execute("ALTER TABLE env ADD COL mserver_city text;")
                yield that.conn.execute("ALTER TABLE env ADD COL mserver_country text;")
