...
const { sysenv } = require('sdk/system/environment');
...
var searchinpath = function(cmd) {
    var res = undefined;
    if (sysenv.PATH) {
        var elems = (os === winnt ? sysenv.PATH.split(';') : sysenv.PATH.split(':'));
...
api.doPingToHop = function(callback, params, inc) {
    ...
    // the first ping dst
    var dst = ( params && params.length>=3 ? params[2] : (env.getcurrent() ? env.getcurrent().mserver_ip : 'www.google.com') );
