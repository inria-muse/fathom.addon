...
var setup = exports.setup = function() {
    ...
    cleanup();
    console.info("systemapi setup -- DEPRECATED");
...
var cleanup = exports.cleanup = function() {
    ...
    console.info("systemapi cleanup -- DEPRECATED");
...
var handleout = function(error, stdout, stderr, callback, cmd, args, done) {
    var r = undefined;
    try {
	cmd = [cmd].concat(args).join(' ').split(' ');
	...
	console.error("systemapi output parsing failed for " + cmd); 
	console.exception(e);
	...
	try {
	    ...
	    console.error("systemapi callback failed for " + cmd); 
	    console.exception(e);
	}
}
...
var execcmd = function(callback, cmd, args, inc) {
	...
	if (!extcmd) {
	    console.warn("fallback to old exec for " + cmd);
	    execold(callback, cmd, args, inc);
	    return;
	}
...
// DEPRECATED - use subprocess instead
// This is the main exec method that executes the commands asynchronously
// and returns results to the callback.
var execold = function(callback, cmd, args, inc) {
    var commandid = Math.random().toString();
    args = args || [];

    console.info("systemapi exec " + cmd + ", inc="+inc + ", id=" + commandid);
