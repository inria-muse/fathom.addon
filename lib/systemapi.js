...
api.getRoutingTable = function(callback, params, inc) {
    ...
    execcmd(function(resroutes, done) {
        ...
        if (os === darwin) {
            ...
            resroutes.result.defaultgateway = _.pick(_.find(resroutes.result.routes, function(r) { return r.defaultroute; }), 'gateway', 'iface');            
            if (resroutes.result.defaultgateway) {
            ...
            // as a fallback pick (one of the possible) default route 
            resroutes.result.defaultgateway = _.pick(_.find(resroutes.result.routes, function(r) { return r.defaultroute; }), 'gateway', 'iface');
            return _cb(resroutes, true);
        }
        ...
        execcmd(function(res, done) {
            if (!res.error && res.result.gateway) {
                resroutes.result.defaultgateway = res.result;
                var iface = _.find(resroutes.result.routes, function(r) { 
                    return r.gateway === res.result.gateway; 
                });
                resroutes.result.defaultgateway = _.extend(resroutes.result.defaultgateway, iface);
            }
            return _cb(resroutes, true);
        }, cmd, args, undefined, false);
    }, cmd, args, undefined, false);
