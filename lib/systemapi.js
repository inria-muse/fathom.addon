...
api.getRoutingTable = function(callback, params, inc) {
    ...
    execcmd(function(resroutes, done) {
        ...
        tmp = _.find(tmp, function(r) { return r.defaultroute; });
        if (tmp) {
            resroutes.result.defaultgateway = _.pick(tmp, 'gateway', 'iface');
        }
        ...
        if (os === darwin) {
            ...
            execcmd(function(res, done) {
                if (!res.error && res.result.gateway) {
                    ...
                    var tmp = _.find(resroutes.result.routes, function(r) { 
