...
var extname = "extensions."+self.id+".sdk.console.logLevel";
...
console.info("loading " + self.name + " addon v=" + self.version + " devmode=" + ss.storage['devmode']);
...
exports.setup = function(install, upgrade) {
	...
	pageMod.PageMod({
		...
		onAttach: function(worker) {
			if (userPrefs[consts.PAGELOAD] && worker.tab) {
	...
	pageMod.PageMod({
		...
		contentScriptWhen: 'end',
		onAttach: function(worker) {
			if (userPrefs[consts.MEDIALOAD] && worker.tab) {
				var page = worker.tab.id+'.'+worker.tab.url;
	...
	if (userPrefs[consts.FATHOMAPI]) {
    	console.log('fathom API enabled');
    ...
    sprefs.on("", function(prefname) {
    	...
    	if (prefname === consts.BASELINE) {
		    ...
		    if (userPrefs[consts.FATHOMAPI]) {
		    	...
		    	console.log('fathom API disabled');
		    	apiMod.destroy();
		    	apiMod = undefined;
		    }
		}
...
exports.cleanup = function(uninstall) {
    ...
    if (uninstall) {
		...
		// FIXME: all these take a bit time ...
		upload.addUploadItem("fathomstats", {
			ts : ts.getTime(),
			timezoneoffset : ts.getTimezoneOffset(),
			action : "uninstall",
