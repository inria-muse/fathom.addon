...
var handlereq = function(worker) {
	...
	return sendres(req, error(err,msg), true);
    };
	...
	if (req.module === 'internal') {
		...
		var handleuserres = function(userok) {
			...
			return senderrresp(req,"notallowed",manifest.appname); 
		    }
		...
		dialogs.showUploadDialog(function(userok) {
		    if (userok) {
			upload.addUploadItem(manifest.appname, obj);
		    }
