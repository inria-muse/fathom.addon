...
var handlereq = function(worker) {
    if (!worker || !worker.tab) // latter seems to happen sometimes ... ?!?
	return function() {};
	...
	if (!pageworkers[winid]) return; // unregistered
	if (done === undefined)
	    done = true;
	...
	if (req.module === 'internal') {
		...
		if (obj[config.BASELINE_UPLOAD] !== undefined) {
		    userPrefs[config.BASELINE_UPLOAD] = (obj[config.BASELINE_UPLOAD] ? config.UPLOAD_ALWAYS : config.UPLOAD_NEVER);
		}

		if (obj[config.PAGELOAD_UPLOAD] !== undefined) {
		    userPrefs[config.PAGELOAD_UPLOAD] = (obj[config.PAGELOAD_UPLOAD] ? config.UPLOAD_ALWAYS : config.UPLOAD_NEVER);
		}
...
pageMod.PageMod({
	...
	worker.port.on("perf", function(p) {
		...
		console.debug("worker.tab is empty",p);
	    }
	});
    }
});
