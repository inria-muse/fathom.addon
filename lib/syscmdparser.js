...
parserfuncs["hostname"] = function(out, args) {
    return (out ? out.trim() : "");
};

parserfuncs["getprop"] = function(out, args) {
    if (os === android)
	return (out ? out.trim() : "");
    else
...
parserfuncs["vm_stat"] = function(out, args) {
    if (os !== darwin)
...
parserfuncs["cat"] = function(out, args) {
    function ip4(val) {
	var addr = [];
	var tmp = (val & 0xFF);
...
parserfuncs["ifconfig"] = function(out, args) {
    var res = [];
...
parserfuncs["netsh"] = function(out, args) {
    out = (out ? out.trim() : "");
    if (os !== winnt)
	...
	while (blocks.length > 0) {
	    ...
	    for (i = 1; i < lines.length; i++) {			
		line = lines[i].trim().replace(/\s+/g, ' ').split(': ');
		if (line[0].toLowerCase().indexOf('dns server')>=0 && line[1].toLowerCase()!=='none')
		    res.nameservers.push(line[1]);
	    }
	}	
	break;
	...
	while (blocks.length > 0) {			
	    ...
	    for (i = 1; i < lines.length; i++) {			
		line = lines[i].trim().replace(/\s+/g, ' ').split(': ');
		switch (line.length == 2 && line[0].toLowerCase()) {
		case 'dhcp enabled':
	...
	break;
    default:
	throw new Error("syscmdparser 'netsh' dunno how to parse '" + args[args.length-1] + "'");		
    }
    return res;
};

parserfuncs["iwconfig"] = function(out, args) {
    if (os !== linux)
    ...
    var line,i;
    for (i = 0; i < lines.length; i++) {
	if (lines[i].indexOf('no wireless extensions'))
	    continue
	...
	if (line[1] === 'IEEE') {
	    res['name'] = line[0];
	    res['version'] = line[2];
	    res['ssid'] = line[3].split(':').replace(/"/gi,'');
	} else {
	    while (line.length > 0) {
		if (line[0].indexOf('Mode:')>=0) {
		    res['mode'] = line[0].split(':')[1]
		}
		if (line[0].indexOf('Frequency:')>=0) {
		    res['freq'] = line[0].split(':')[1]
		}
		if (line[0].indexOf('Point:')>=0) {
		    res['bssid'] = line[1]
		    line.shift()
		}
		if (line[0].indexOf('Rate:')>=0) {
		    res['txrate'] = line[0].split(':')[1]
		}
		if (line[0].indexOf('Tx-Power:')>=0) {
		    res['txpower'] = line[0].split(':')[1]
		}
		if (line[0].indexOf('Quality:')>=0) {
		    res['quality'] = line[0].split(':')[1]
		}
		if (line[0].indexOf('Signal')>=0 && 
		    line[1].indexOf('level:')>=0) 
		{
		    res['signal'] = line[1].split(':')[1]
		}
		if (line[0].indexOf('Noise')>=0 && 
		    line[1].indexOf('level:')>=0) 
		{
		    res['noise'] = line[1].split(':')[1]
		}
...
parserfuncs["systeminfo"] = function(out, args) {
    if (os !== winnt)
...
parserfuncs["networksetup"] = function(out, args) {
    if (os !== darwin)
...
parserfuncs["airport"] = function(out, args) {
    ...
    if (!_.contains(args, "-I") && !_.contains(args, "-s")) { 
...
parserfuncs["ip"] = function(out, args) {
    var res = [];
    var lines = (out ? out.trim() : "").split("\n");
...
parserfuncs["route"] = function(out, args) {
    var res = { routes : [], defaultroute : undefined };
	switch (os) {
	case winnt:
		out = (out ? out.trim() : "");
		if (out.indexOf('Active Routes:')>=0) {
			out = out.split("Active Routes:")[1].trim();
			var lines = out.split("\n");
...
parserfuncs["netstat"] = function(out, args) {
    var res = [];
    var lines = (out ? out.trim() : "").split("\n");
    var i, line;
...
parserfuncs["top"] = function(out, args) {
    var res = { 
	tasks : {
	    total: null,
	    running: null,
...
parserfuncs["nslookup"] = function(out, args) {
    var lines = (out ? out.trim() : "").split("\n");
    var res = {
...
parserfuncs["ping"] = function(out, args) {
    var lines = (out ? out.trim() : "").split("\n");
    var res = {
...
parserfuncs["tracert"] = function(out, args) {
    var lines = (out ? out.trim() : "").split("\n");
    var res = {
	dst: args[args.length-1],
	hops: [],        // { hostname : val, rtt : [], address : val, id : val };
...
parserfuncs["mtr"] = function(out, args) {
 ...
 *  the given os.
 */
var parse = exports.parse = function(err, stdout, stderr, cmd, args) {
    if (!cmd)
	...
	res.result = parserfuncs[cmd](stdout, args);
    }
