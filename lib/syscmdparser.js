...
parserfuncs["netsh"] = function(out, cmd) {
	...
	break;
    default:
	throw new Error("syscmdparser 'netsh' dunno how to parse '" + cmd[cmd.length-1] + "'");		
    }
    return res;
};
...
parserfuncs["systeminfo"] = function(out, cmd) {
    if (os !== winnt)
...
parserfuncs["netstat"] = function(out, cmd) {
    ...
    } else if (_.intersection(cmd, ['-e']).length === 1 && os === winnt) {
	// interface statistics
	var obj = {
	    name : 'ethernet',
	    rx : {
...
parserfuncs["tracert"] = function(out, cmd) {
    var lines = (out ? out.trim() : "").split("\n");
    var res = {
	dst: cmd[cmd.length-1],
	hops: [],        // { hostname : val, rtt : [], address : val, id : val };
 ...
 *  the given os.
 */
var parse = exports.parse = function(err, stdout, stderr, cmd) {
    if (!cmd)
	...
	{
	    console.info("syscmdparser error",code,stdout,stderr);
