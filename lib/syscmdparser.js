...
parserfuncs["iwlist"] = function(out, args, params) {
    ...
    _.each(blocks, function(b) {
        if (b.indexOf('Scan completed')>0) {
            var cells = b.replace(/\s+/g, ' ').split('Cell ');
            for (var i = 1; i < cells.length; i++) {
                ...
                var lines = cells[i].split('\n');
                for (var j = 0; j < j.length; j++) {
                    if (lines[j].indexOf('Address: ')>=0) {
                        wifinet.bssid = lines[j].split(': ')[1];
                    } else if (lines[j].indexOf('Channel:')>=0) {
                        wifinet.channel = parseInt(lines[j].split(':')[1]);
                    } else if (lines[j].indexOf('ESSID:')>=0) {
                        wifinet.ssid = lines[j].split(':')[1].replace(/"/gi,'');
                    } else if (lines[j].indexOf('Encryption key')>=0) {
                        wifinet.auth = lines[j].split(':')[1];
                    } else if (lines[j].indexOf('Bit Rates:')>=0) {
                        _.each(lines[j].split(':')[1].trim().split(';'), function(r) { 
                            r = r.split(' ');
                            wifinet.rates.push(parseFloat(r));
                        });
                    } else if (lines[j].indexOf('Extra:')>=0 || lines[j].indexOf('IE: ')>=0) {
                        continue; // ignore
                    } else {
                        var tmp = lines[j].split(':');
                        wifinet[tmp[0].trim().toLowerCase()] = tmp[1].trim().toLowerCase();
                    }
                }
                res.push(wifinet);
            };
