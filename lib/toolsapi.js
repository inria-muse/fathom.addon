...
var discovery = exports.discovery = function(callback, timeout, protocols, manifest) {
	...
	protocols = ['local','route','internet','upnp','mdns', 'fathom'];
   }
	...
	case "upnp":
		...
		protoapi.exec(function(dev, doneflag) {
			...
			var name = (dev.xml ? dev.xml.friendlyName : undefined);

			if (!name && dev.iswin)
			    name = "Windows Device";
			else if (!name && dev.islinux)
			    name = "Linux Device";
			else if (!name && dev.isdarwin)
			    name = "Mac Device";
			else if (!name)
			    name = "Network Device";
