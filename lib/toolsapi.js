    ...
    socketapi.exec(function(res, done) {
        ...
        socketapi.exec(function(res, done) { 
           return callback((!res.error && res.pings.length > 0), true);
            ...
            submodule : 'ping',
            method : 'start', 
            params : [config.get('mserver', 'ipv4'), pcfg],
            id : 2
        }, apimanifest);
...
var lookupMAC = exports.lookupMAC = function(callback, mac) {    
    ...
    Request({
        url: config.get("api", "url")+"/mac/"+mac,
...
var lookupIP = exports.lookupIP = function(callback, ip) {
    Request({
        url: config.get("api", "url")+"/fulllookup" + (ip ? "/"+ip : ""),
        onComplete: function(response) {
            if (response.status == 200 && !response.error) {
                callback(response.json);
            } else {
            ...
            lookupIP(function(res) {
                if (res && !res.error) {
                    ...
                    var node = new Node(
                        p,
                        true, // reachable
                        config.get("mserver", "ipv4"), // use some fixed address that will not appear as other node's address
                        undefined,
                        res);
                    addnode(p, node, true);
                    return;                    
                    ...
                    for (var i = 0; i < _.size(rifaces); i++) {
                        ...
                        systemapi.exec(function(res) {
                            ...
                            systemapi.exec(function(res2) {
                                ...
                                params : [config.get("mserver", "ipv4"), {
                                  count : 3, 
                                  timeout : timeout, 
                                  interval : 0.5,
