    ...
    _.each(protocols, function(p) {
                    ...
                    _.each(rifaces, function(routes, name) {
                        ...
                        systemapi.exec(function(res) {
                            ...
                            systemapi.exec(function(res2) {
                                ...
                                // done!
                                addnode(p, node, (cnt === _.size(rifaces)));
        ...
        case "ping":
            ...
            systemapi.exec(function(ifaces) {
                ...
                var ecnt = 0;
                _.each(ifaces.result, function(iface, index) {
                    if (iface.type === 'loopback' || 
                        !iface.broadcast || 
                        iface.broadcast.indexOf('169.') == 0) 
                    {
                        ecnt += 1;
                        if (ecnt === _.size(ifaces.result)) {
                           addnode(p,undefined,true); // signal done
                        }
                        return; // skip
                    }
                    ...
                    systemapi.exec(function(res, doneflag) {
                        if (res.error || !res.result.alt || _.size(res.result.alt.length)==0) {
                            ecnt += 1;
                            if (ecnt === _.size(ifaces.result)) {
                               addnode(p,undefined,true); // signal done
                            }
                        ...
                        _.each(res.result.alt, function(r, index2) {
                            var lastitem = (index === _.size(ifaces.result)-1 && index2 === _.size(res.result.alt.length)-1);
        ...
        case "arptable":
            systemapi.exec(function(res) {
                ...
                _.each(res.result, function(raw, index) {
                    var lastitem = (index == res.result.length-1);
