...
var doaggregate = function(ts) {
	...
	// FIXME: this may take a while after FF restarts
	// as we are checking all intervals (including when
	// ff was shut down ...)
...
var getnetworkenvdb = exports.getnetworkenvdb = function(callback, env) {
	...
	try {
	    ...
	    console.debug("baseline env",env);
		...
		console.log("baseline lookup public IP for " + env.env_id);
...
var inittables = function() {
	...
	let initdone = yield conn.tableExists('baseline');
	let schemaversion = conn.schemaVersion;
	console.log("baseline db schema="+schemaversion+" init="+initdone);

	// check if we need to update the env table (v.2.0.1 -> v.2.0.2)
	if (initdone && schemaversion == 0) {
	    let env = yield conn.execute("SELECT * FROM env LIMIT 1;");
	    if ((!env || env.length == 0)) {
		// re-create with the new schema
		yield conn.execute("DROP TABLE env;");
		console.info("baseline db redo " + SQL_CREATE_ENV);
		yield conn.execute(SQL_CREATE_ENV);
		conn.schemaVersion = 1;

	    } else if (env[0]['public_ip'] === undefined) {
		// has some data, update the schema
		console.info("baseline db update to schema 1");
		yield conn.execute("ALTER TABLE env ADD public_ip text;");
		yield conn.execute("ALTER TABLE env ADD country text;");
		yield conn.execute("ALTER TABLE env ADD city text;");
		yield conn.execute("ALTER TABLE env ADD isp text;");
		yield conn.execute("ALTER TABLE env ADD net_desc text;");
		yield conn.execute("ALTER TABLE env ADD as_number text;");
		yield conn.execute("ALTER TABLE env ADD as_desc text;");
		yield conn.execute("ALTER TABLE env ADD lookup_ts integer;");
		conn.schemaVersion = 1;
	    }
	}
