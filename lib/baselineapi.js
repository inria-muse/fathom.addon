...
var domeasurements = exports.domeasurements = function(cb, ts, runid) {
    ...
    // for traceroutes, do not run everytime
    var condexec = function(cfg) {
        if ((runid-1)%config.BASELINE_TR_FREQ === 0) {
    ...
    all([
        ...
        toolsapi.execp({ method : 'lookupHostname', 
                         params: [config.MSERVER_HOSTNAME_FR] })
        ...
        baseline.mem = results[7];
        baseline.traffic = results[8];
        baseline.wifi = results[9];        
        baseline.rtt.rtt0 = results[10];
        baseline.rtt.rtt1 = results[11];
        baseline.rtt.rtt2 = results[12];
        baseline.rtt.rtt3 = results[13];
        baseline.dns = results[14];
        cb(baseline);
    }, function err(reason) {
        cb({ error : reason});
    });
