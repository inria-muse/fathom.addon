...
const flags = Ci.nsIDNSService.RESOLVE_DISABLE_IPV6 | Ci.nsIDNSService.RESOLVE_CANONICAL_NAME;
...
const strings = {
    ...
    "manifest_dst_ip" : "Network host '%s [%s]'",
    "manifest_dst_ip_proto" : "Network host '%s [%s]' using %s protocol",
    "manifest_dst_ip_port" : "Network host '%s [%s]' on port %d",
    "manifest_dst_ip_proto_port" : "Network host '%s [%s]' on port %d using %s protocol"
};
...
var URI = function(str) {
    ...
    if (this.hostname.indexOf('{') !== 0 && !ipaddr.isValid(this.hostname)) {
    ...
    default:
        if (this.port === '*' && this.proto === '*') {
            ...
            this.desc = loc(this.descname, this.hostname, this.host, this.proto);
        } else if (this.port !== '*' && this.proto === '*') {
            this.descname += '_ip_port';
            this.desc = loc(this.descname, this.hostname, this.host, this.port);
        } else if (this.port !== '*' && this.proto !== '*') {
            this.descname += '_ip_proto_port';
            this.desc = loc(this.descname, this.hostname, this.host, this.port, this.proto);
        } 
    break;
    }
};
 ...
 */
 var checkDstPermission = function(dstobj, manifest) {
    ...
    if (!ipaddr.isValid(dstobj.host)) {
        // see if we have resolved this already (match by name)
        var predefdst = _.find(manifest.destinations, function(d) {
            return d.hostname === dstobj.hostname;
        });

        if (!predefdst) {
            // Most APIs require IP address, do the resolution here
            // NOTE: this is synchronous .. 
            try {
                var r = dnsservice.resolve(dstobj.host, flags);
                if (r.hasMore()) {
                    dstobj.host = dnsservice.resolve(dstobj.host, flags).getNextAddrAsString();
                } else {
                    console.error("could not resolve host IP: " + dstobj.host);
                    return false;
                }
            } catch (e) {
                console.error("could not resolve host IP: " + dstobj.host, e);
                return false;            
            }
        } else {
            dstobj.host = predefdst.host;
        }
    }

    if (!manifest.isaddon) {
        // find the host from discovered neighbors
        var neighp = _.find(_.keys(manifest.neighbors), function(p) {
    ...
    // else addon page - allow everything
    return true;
};
 ...
 */
 var checkDstServerPermission = function(dstobj, manifest) {
    console.info("security server manifest check",dstobj);
    if (!manifest.isaddon)
