...
var exec = exports.exec = function(callback, req, manifest) {
    ...
    console.debug("socketapi securitycheck="+checkok + " for " + 
          dst.proto+"://"+dst.host+":"+dst.port);

    if (!checkok) {
        return callback(error("destinationnotallowed", 
                      dst.proto+"://"+dst.hostname+":"+dst.port));
    }

    if (dst.host && !security.checkDstServerPermission(dst, manifest)) {
    ...
    if (req.method.toLowerCase().indexOf('open')>=0 || 
        ...
        sw = {
            ...
            noinitresp : (req.module === 'tools' && req.method === 'start') // do not send sid for ping clis
        };
