...
var start = exports.start = function(cb) {
    ...
    request.onupgradeneeded = function(event) {
        console.log("upload database open onupgradeneeded");
    ...
    request.onsuccess = function(event) {
        console.log("upload database open onsuccess");
 ...
 */
 ...
 var schedNext = function(backoff, first) {
    ...
    uploadtimer = timers.setTimeout(uploadTimerTask, next*1000);
};
...
var uploadItems = exports.uploadItems = function(cb, url) {
    ...
    var ts = new Date();
    var uploadurl = url || config.UPLOAD_URL;
    if (uploadtimer)
        timers.clearTimeout(uploadtimer);
...
var addUploadItem = exports.addUploadItem = function(collection, objs, cb) {
    ...
    var addall = function(env) {
        ...
        var loop = function() {
            ...
            } else if (ss.storage['upload']['queue_size'] >= config.UPLOAD_MAX_QUEUE) {
