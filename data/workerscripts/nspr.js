...
/**
 * @fileoverfiew NSPR library wrapper.
 *
 * @author Anna-Kaisa Pietilainen <anna-kaisa.pietilainen@inria.fr> 
 */

// Library loading wrapped in a function so we don't leak the
// helpers into the global namespace
NSPR = (function() {
    var openLibrary = function(name, path) {
    // jsamuel: With my current testing, a standard Firefox installation on
    // 32-bit Windows 7 fails to load the library when given the full path 
    // but will load it given only the name. However, on 64-bit Linux with 
    // a locally installed Firefox the library doesn't load if we provide 
    // only the name.
    ...
    if (!worker || !worker.nsprname || !worker.nsprpath)
        throw "global worker object not initialized!";
    if (!NSPR)
        throw "global NSPR object not initialized!";

    var lib = openLibrary(worker.nsprname, worker.nsprpath);    

    NSPR.closeLib = function() {
        var PR_Cleanup = lib.declare("PR_Cleanup",
                ctypes.default_abi,
    ...
    var getFirstNetAddrPad = function() {
        try {
            try {
                ...
                // The largest total size we have seen for the PRSocketOptionData 
                // struct is 232 bytes, so using 260 bytes should be enough and
                // lets us populate the bytes field with sequential values from 
                // 0 to 255.
                ...
                var PR_Close = lib.declare("PR_Close",
                                    ctypes.default_abi,
                                    ctypes.int32_t,
                                    PRFileDesc.ptr);

            } catch (e) {
                throw 'error in declarations: ' + e;
            }

            // Figure out if there is padding in PRSocketOptionData between 
            // the option field and a following PRNetAddr (the value union's
            // mcast_if member for a PR_SockOpt_McastInterface 
            // PR_GetSocketOption call).
            var fd = PR_OpenUDPSocket(PR_AF_INET);
            if (fd < 0) {
                throw 'failed to open udp socket: ' + fd;
            }
        
            var opt = new SocketOptionData();       
            var getOffset = function() {
                // Initialize the non-option bytes of our PRSocketOptionData 
                // so that they contain byte values corresponding to their
                // position (0 to 255). We'll check for what has changed 
                // after NSPR function calls modify the data.
                ...
                // We expect no padding (offset is 4) or 4 bytes of 
                // padding (offset is 8).
            ...
            // the values we initialized. However, we expected zeros to 
            // be written, not uninitialized data, so this is probably not 
            // necessary.
            if (offset == -1) {
                offset = getOffset();
            }
            ...
            // The 4 bytes immediately preceding the ip address are not struct 
            // padding but rather the inet.family and inet.port fields. 
            // Anything else is the padding between the 4 bytes (we assume 
            // the enum will be represented by 4 bytes) of PRSockOption and 
            // the first PRNetAddr of the PRMcastRequest
    ...
    NSPR.types = {      
        ...
        // padding affects structs that have unions like PRNetAddr and 
        // PRSocketOptionData PRSocketOption data has been split into 
        // PRSocketOption, PRMulticastSocketOptionData and other socket 
        // option structs that have not been defined, like 
        // PRLingerSocketOptionData 
        ...
        // We use this for PRSocketOptionData when the value union's 
        // member is a PRUintn, PRBool, or PRSize. Each of these end up 
        // being the size of an integer on the current platform. 
        // Thus, we use "ctypes.int" which will
    ...
    // We don't know the true size of PRNetAddr but only the IP address 
    // field of the ifaddr member is ever used.
    ...
    /*
     * As IpAddrAny is all zeros and only the ip field of ifaddr is read 
     * we can just set all bytes of ifaddr to zero. We don't need to know 
     * where the ip field really is.
     */
